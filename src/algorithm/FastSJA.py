...
class Optimizer(cplex.callbacks.SimplexCallback):
    def __call__(self):
        ...
        if self.LP_type == 0:
            ...
            if stop_duals[self.tau_index] == 1 or abs(duals[self.tau_index] - value) <= self.tau or early_stops[
                self.tau_index] == 1:
                self.abort()
        else:
...
def RunAlgorithm():
    ...
    primals = multiprocessing.Manager().dict()
    duals = multiprocessing.Manager().dict()
    stop_primals = multiprocessing.Manager().dict()
    stop_duals = multiprocessing.Manager().dict()
    early_stops = multiprocessing.Manager().dict()
    ...
    for i in range(processor_num):
        ...
        # print(i)
        threads.append(multiprocessing.Process(target=ThresholdRunAlgorithm, args=(
            base, downward_sensitivity, arrangement_of_tau_ids[i], 1, tilde_Q_tau)))
        threads[i].start()
    for i in range(processor_num):
        threads[i].join()
...
def processFastSJA(result, e=1, b=0.1, gs=1000000, afactor=0, p_num=1):
